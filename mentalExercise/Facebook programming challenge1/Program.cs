 ...
 *  Question 1 / 1
    ...
    Pick your language C C++ Java PHP Ruby Python Perl Haskell
    ...
    For more details, please check http://www.interviewstreet.com/recruit/challenges/faq/view#stdio
Idea: Create a simple binary tree that represent the dependencies of balances - doing a post order traversal guarantees that children are calculated
 * before the parents. Issue: there may be more than one child balance in right or left. Need to use graph or a "sibling" link.
  */
namespace Facebook_programming_challenge1
{
    struct Balance{
        double rightWeight;
        double leftWeight;
        double rightBalance;
        double leftBalance;
        bool isBalanceProcessed;
    };
    class BalanceDependencyTree{
        int BalanceNum;
        BalanceDependencyTree rightChildBalance;
        BalanceDependencyTree leftChildBalance;
        public void PostOrderTraverse(BalanceDependencyTree root)
        {
            if(root==null) return;
            if(root.rightChildBalance==null) PostOrderTraverse(root.rightChildBalance);
            if(root.leftChildBalance==null) PostOrderTraverse(root.leftChildBalance);
            Console.WriteLine(root.BalanceNum);
        }
        public void PopulateTreeFromInput()
        {

        }
    }

  /*  class BalanceWeightCheck
    {
        int nBalance;
        //assume right and left can have only one balance
        
        Balance[] balanceArrary;
        public balanceWeightCheck(int numBalance)
        {
            balanceArrary = new Balance[numBalance];
        }
        public int ProcessBalanceWeightCheck()
        {
   ...
   * */
    ...
    {
        //members
        BalanceDependencyTree m_tree = new BalanceDependencyTree();
        ...
        static void Main(string[] args)
        {
            ...
            BalanceWeightCheck App = new BalanceWeightCheck();
            App.GetFileName(ref strFileName);
            ...
            //Read the text file and populate the Tree
