...
//Solve 8 tile problem using AStar Alg
namespace eight_tile_problem
{
    class Board: IComparable<Board>
    {
        ...
        public Board()
        {
           ...
           // tiles = new int[10] { -1, 5, 2, 8, 3, -1, 7, 1, 4, 6 };
            //tiles = new int[10] { -1, 1, 2, 3, 0, -1, 4, 5, 6,7 };
            //tiles = new int[10] { -1, 4, 1, 3, -1, 2, 6, 7, 5, 8 };
            //tiles = new int[10] { -1, 1, 2, 3, 4, 5,-1, 7, 8,6 };
            //TODO R: This has a solution, but my alg says there is none.. {right, up, left, left, down, right, right, down}
            tiles = new int[10] { -1, 2, 3, 5, 1, -1, 4,7,8,6 };
            ManhattanDistance = 0;
            distFromBegin = 0;
        }
        ...
        public override int GetHashCode()
        {
            ...
            {
                if(tiles[i] !=-1)
                    hashVal += (i * i * i* tiles[i]) % Int32.MaxValue;
            }
            return hashVal;
        }
        //TODO R: GetPossibleBoardConfigurations
        public Board[] GetChildBoards()
        {
            ...
            int blankIndex =0;
            int i;
            //TODO R: can we just store the blank values instead of calculating everytime
